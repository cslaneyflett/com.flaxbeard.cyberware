...
public class CyberwareContent
{
	...
	public static MobEffectInstance neuropozyneEffect;
	public static MobEffectInstance rejectionEffect;
	...
	public static void register(RegisterEvent event)
	{
		...
		{
			EntityRegistry.registerModEntity(new ResourceLocation(Cyberware.MODID + ":cyberzombie"),
				EntityCyberZombie.class, "cyberzombie", 0, Cyberware.INSTANCE, 80, 3,
				true
			);
		...
		component = new ItemCyberwareBase("component", "actuator", "reactor", "titanium", "ssc", "plating",
			"fiberoptics", "fullerene", "synthnerves", "storage", "microelectric"
		);

		if (CyberwareConfig.INSTANCE.ENABLE_CLOTHES.get())
		{
			shadesMat1 = EnumHelper.addArmorMaterial("SHADES", "cyberware:vanity", 5, new int[]{1, 2, 3, 1}, 15,
				SoundEvents.ITEM_ARMOR_EQUIP_GENERIC, 0.0F
			);
			shadesMat1.repairMaterial = new ItemStack(Item.getItemFromBlock(Blocks.GLASS), 1,
				OreDictionary.WILDCARD_VALUE
			);
			shadesMat2 = EnumHelper.addArmorMaterial("SHADES2", "cyberware:vanity_2", 5, new int[]{1, 2, 3, 1}, 15,
				SoundEvents.ITEM_ARMOR_EQUIP_GENERIC, 0.0F
			);
			shadesMat2.repairMaterial = new ItemStack(Item.getItemFromBlock(Blocks.GLASS), 1,
				OreDictionary.WILDCARD_VALUE
			);
			...
			jacketMat = EnumHelper.addArmorMaterial("JACKET", "cyberware:jacket", 5, new int[]{1, 2, 3, 1}, 15,
			...
			trenchMat = EnumHelper.addArmorMaterial("TRENCHCOAT", "cyberware:trenchcoat", 5, new int[]{1, 2, 3, 1}, 15
				, SoundEvents.ITEM_ARMOR_EQUIP_LEATHER, 0.0F);
		...
		{
			katanaMat = EnumHelper.addToolMaterial(
				"KATANA",
		...
		bodyPart = new ItemBodyPart(
			"body_part",
			new EnumSlot[]{EnumSlot.EYES, EnumSlot.CRANIUM, EnumSlot.HEART, EnumSlot.LUNGS, EnumSlot.LOWER_ORGANS,
				EnumSlot.SKIN, EnumSlot.MUSCLE, EnumSlot.BONE, EnumSlot.ARM, EnumSlot.ARM, EnumSlot.LEG,
				EnumSlot.LEG},
			new String[]{"eyes", "brain", "heart", "lungs", "stomach", "skin", "muscles", "bones", "arm_left",
		...
		cybereyeUpgrades = new ItemCybereyeUpgrade("cybereye_upgrades", EnumSlot.EYES,
			new String[]{"night_vision", "underwater_vision", "hudjack",
				"targeting", "zoom"}
		);
		...
		brainUpgrades = new ItemBrainUpgrade("brain_upgrades", EnumSlot.CRANIUM,
			new String[]{"cortical_stack", "ender_jammer", "consciousness_transmitter"
				, "neural_contextualizer", "matrix", "radio"}
		);
		...
		lowerOrgansUpgrades = new ItemLowerOrgansUpgrade("lower_organs_upgrades", EnumSlot.LOWER_ORGANS,
			new String[]{"liver_filter", "metabolic", "battery",
				"adrenaline"}
		);
		...
		RecipeHandler.addShapelessOreRecipe(
		...
		if (CyberwareConfig.INSTANCE.SURGERY_CRAFTING.get())
		{
		...
		if (CyberwareConfig.INSTANCE.INT_BOTANIA.get() && Loader.isModLoaded(BotaniaIntegration.MOD_ID))
		{
			BotaniaIntegration.preInit();
		}

		if (CyberwareConfig.INSTANCE.INT_TOUGH_AS_NAILS.get() && Loader.isModLoaded(ToughAsNailsIntegration.MOD_ID))
		{
			ToughAsNailsIntegration.preInit();
		}

		if (CyberwareConfig.INSTANCE.INT_ENDER_IO.get() && Loader.isModLoaded(EnderIOIntegration.MOD_ID))
		{
		...
		if (CyberwareConfig.INSTANCE.MOBS_ENABLE_CYBER_ZOMBIES.get())
		{
			...
			Cyberware.logger.info(String.format(
				"Adding CyberZombies spawning to the following biomes: %s",
				strBiomes.toString()
			));
			EntityRegistry.addSpawn(
				EntityCyberZombie.class,
				CyberwareConfig.INSTANCE.MOBS_CYBER_ZOMBIE_WEIGHT.get(),
				CyberwareConfig.INSTANCE.MOBS_CYBER_ZOMBIE_MIN_PACK.get(),
				CyberwareConfig.INSTANCE.MOBS_CYBER_ZOMBIE_MAX_PACK.get(),
				MobCategory.MONSTER,
	...
	public static class NumItems implements WeightedEntry
	{
		public int num;
		private final Weight weight;

		public NumItems(Weight weight, int num)
		{
			this.weight = weight;
			this.num = num;
		}
	...
	public static class ZombieItem implements WeightedEntry
	{
		...
		public boolean equals(Object target)
		{
			...
			return (stack == stack2
				...
				&& CyberwareItemMetadata.identical(stack, stack2)
	...
	//		if (meta != -1) {
	//			try {
	//				if (CyberwareItemMetadata.get(itemStack) < 0) throw new RuntimeException();
	//			} catch (final Exception exception) {
	//				throw new RuntimeException(
	//					String.format("Failed to get mod item for %s@%d",
	//						registryName, meta
	//					));
	//			}
	//		}

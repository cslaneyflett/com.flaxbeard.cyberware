...
public class RenderCyberlimbHand
{
	...
	public void renderItemInFirstPerson(AbstractClientPlayer p_187457_1_, float p_187457_2_, float p_187457_3_,
										EnumHand p_187457_4_, float p_187457_5_, @Nullable ItemStack p_187457_6_,
										float p_187457_7_)
	{
			...
			if (p_187457_1_.isHandActive() && p_187457_1_.getItemInUseCount() > 0 && p_187457_1_.getActiveHand() == p_187457_4_)
			{
				...
				switch (p_187457_6_.getItemUseAction())
				{
					...
					case BOW:
						...
						float f5 =
							(float) p_187457_6_.getMaxItemUseDuration() - ((float) this.mc.player.getItemInUseCount() - p_187457_2_ + 1.0F);
						float f6 = f5 / 20.0F;
	...
	private void renderArmFirstPerson(float p_187456_1_, float p_187456_2_, EnumHandSide p_187456_3_)
	{
		...
		float f1 = Mth.sqrt(p_187456_2_);
		float f2 = -0.3F * Mth.sin(f1 * (float) Math.PI);
		float f3 = 0.4F * Mth.sin(f1 * ((float) Math.PI * 2F));
		float f4 = -0.4F * Mth.sin(p_187456_2_ * (float) Math.PI);
		GlStateManager.translate(f * (f2 + 0.64000005F), f3 + -0.6F + p_187456_1_ * -0.6F, f4 + -0.71999997F);
		GlStateManager.rotate(f * 45.0F, 0.0F, 1.0F, 0.0F);
		float f5 = Mth.sin(p_187456_2_ * p_187456_2_ * (float) Math.PI);
	...
	private void transformFirstPerson(EnumHandSide p_187453_1_, float p_187453_2_)
	{
		...
		float f = Mth.sin(p_187453_2_ * p_187453_2_ * (float) Math.PI);
		GlStateManager.rotate((float) i * (45.0F + f * -20.0F), 0.0F, 1.0F, 0.0F);
		float f1 = Mth.sin(Mth.sqrt(p_187453_2_) * (float) Math.PI);
		GlStateManager.rotate((float) i * f1 * -20.0F, 0.0F, 0.0F, 1.0F);
		GlStateManager.rotate(f1 * -80.0F, 1.0F, 0.0F, 0.0F);
		GlStateManager.rotate((float) i * -45.0F, 0.0F, 1.0F, 0.0F);
	}

	public void renderItemSide(LivingEntity entitylivingbaseIn, ItemStack heldStack,
										  ...
										  ItemStack p_187465_4_)
	{
		...
		float f1 = Mth.sqrt(p_187465_3_);
		float f2 = Mth.sin(f1 * (float) Math.PI);
		float f3 = -0.5F * f2;
		float f4 = 0.4F * Mth.sin(f1 * ((float) Math.PI * 2F));
		float f5 = -0.3F * Mth.sin(p_187465_3_ * (float) Math.PI);
	...
	{
		float f = Mth.sqrt(p_187463_3_);
		float f1 = -0.2F * Mth.sin(p_187463_3_ * (float) Math.PI);
	...
	private void renderMapFirstPerson(ItemStack stack)
	{
		...
		if (!stack.isEmpty())
		{
			MapData mapdata = Items.FILLED_MAP.getMapData(stack, Minecraft.getInstance().level);
	...
	private float getMapAngleFromPitch(float pitch)
	{
		...
		f = -Mth.cos(f * (float) Math.PI) * 0.5F + 0.5F;
		return f;
	}
}

...
public class GuiEngineeringTable // extends GuiContainer
{
	...
	//	}
	//
	//	private static final ResourceLocation ENGINEERING_GUI_TEXTURES = new ResourceLocation(Cyberware.MODID + ":textures" +
	//		"/gui/engineering" +
	//		".png");
	...
	//	{
	//		super(new ContainerEngineeringTable(Minecraft.getInstance().player.getCachedUniqueIdString(), playerInventory,
	...
	//		fontRenderer.drawString(playerInventory.getDisplayName().getUnformattedText(), offset + 8, ySize - 96 + 2,
	//			4210752
	//		);
	//
	//		next.visible = prev.visible =
	//			(archive() != null && ((ContainerEngineeringTable) inventorySlots).archiveList.size() > 1);
	//		nextC.visible = prevC.visible =
	//			(componentBox() != null && ((ContainerEngineeringTable) inventorySlots).componentBoxList.size() > 1);
	//
	...
	//					chance = Math.min(
	//						100F,
	//						CyberwareConfig.INSTANCE.ENGINEERING_CHANCE.get() * 5F * (1F - (tileEntityEngineeringTable.slots.getStackInSlot(0).getItemDamage() * 1F / tileEntityEngineeringTable.slots.getStackInSlot(0).getMaxDamage()))
	//					);
	//				}
	//				tooltip = new String[]{I18n.get("cyberware.gui.destroy"),
	//					I18n.get("cyberware.gui.destroy_chance", Math.round(chance * 100F) / 100F + "%")};
	//			} else
	//			{
	//				tooltip = new String[]{I18n.get("cyberware.gui.destroy")};
	//			}
	...
	//		{
	//			drawHoveringText(Arrays.asList(I18n.get("cyberware.gui.to_destroy")), mouseX - i, mouseY - j,
	//				fontRenderer
	//			);
	//		}
	//		if (isPointInRegion(offset + 15, 53, 16, 16, mouseX, mouseY) && tileEntityEngineeringTable.slots.getStackInSlot(1).isEmpty())
	//		{
	//			drawHoveringText(Arrays.asList(I18n.get("cyberware.gui.paper")), mouseX - i, mouseY - j, fontRenderer);
	//		}
	//		if (isPointInRegion(offset + 115, 53, 16, 16, mouseX, mouseY) && tileEntityEngineeringTable.slots.getStackInSlot(8).isEmpty())
	//		{
	//			drawHoveringText(Arrays.asList(I18n.get("cyberware.gui.blueprint")), mouseX - i, mouseY - j, fontRenderer);
	//		}
	//
	...
	//					{
	//						itemRender.renderItemAndEffectIntoGUI(mc.player, toRender.get(index),
	//							offset + 71 + 18 * (indexSlot % 2),
	//							-1 + 18 * (indexSlot / 2)
	//						);
	//
	//						Font font = toRender.get(index).getItem().getFontRenderer(toRender.get(index));
	//						if (font == null) font = fontRenderer;
	//
	//						itemRender.renderItemOverlayIntoGUI(font, toRender.get(index),
	//							offset + 71 + 18 * (indexSlot % 2),
	//							-1 + 18 * (indexSlot / 2),
	...
	//					itemRender.renderItemAndEffectIntoGUI(mc.player, prod, offset + 181 + 18 * (h % 3),
	//						22 + 18 * (h / 3)
	//					);
	//				}
	//			}
	...
	//		if (button.id == 0)
	//		{
	//			CyberwarePacketHandler.INSTANCE.sendToServer(new EngineeringDestroyPacket(tileEntityEngineeringTable.getPos(), tileEntityEngineeringTable.getLevel().provider.getDimension()));
	//		}
	//
	//		if (button.id == 2)
	//		{
